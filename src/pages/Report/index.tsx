import React, { useRef, useCallback, useState, Component, lazy, useEffect, Children } from 'react';
import ReactPlayer from 'react-player'
import Slider from "react-slick";
import 'react-multi-carousel/lib/styles.css';
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";
import { Statistic, Card, Row, Col, Layout, Descriptions, Radio, Menu, Dropdown, Button, Input, Result } from 'antd';
import Icon, { ArrowUpOutlined, ArrowDownOutlined, DownOutlined, UserOutlined, ArrowRightOutlined, ConsoleSqlOutlined } from '@ant-design/icons';
import { Container, Contents, Background, Jooj, Selecoes, TempoFrame, InfoAluno } from './styles';
...
const result2 = [
  ...
  },
  {
    ...
    name: "Serain",
    ra: "222170274",
    url_frame: "https://lucasserain.s3.us-east-2.amazonaws.com/62a49d64-9c23-4a66-9b33-811ddc5c557e",
    emocao: "Medo"
  },
  {
    cod_frame: "004bdc55",
...
const lool = [
  ...
  },
  {
    ...
    frame: [{
      ...
      dt_cria: "0:20",
      ra: "222170274",
      url_frame: "https://static.maislaser.com.br/media/catalog/product/cache/2/thumbnail/900x900/9df78eab33525d08d6e5fb8d27136e95/r/o/rosto_1_2.jpg",
      emocao: "Medo"
    },
    {
      cod_frame: "004bdc55",
      ...
      dt_cria: "0:20",
      ra: "222170274",
      url_frame: "https://julianafonte.com.br/wp-content/uploads/2018/02/dermato-estetica-macas-do-rosto.jpg",
      emocao: "Tristeza"
    },
    {
      cod_frame: "004bdc55",
      ...
      dt_cria: "0:20",
      ra: "222170274",
      url_frame: "https://i.pinimg.com/originals/8b/37/aa/8b37aa8a10687703735ea9389085adc2.jpg",
      emocao: "Medo"
    }]
  }
...
const getEmocaoPredominante2 = (resultado: any[]) => {
  ...
  const total = countEmocoes[0].countEmocao + countEmocoes[1].countEmocao + countEmocoes[2].countEmocao + countEmocoes[3].countEmocao + countEmocoes[4].countEmocao + countEmocoes[5].countEmocao + countEmocoes[6].countEmocao;
  let maior = 0;
  let predominante = "";
...
const Report = () => {
  ...
  const getStudentInfo = async (studentStr: any) => {
    ...
    await fetch(`https://6099bb140f5a13001721992c.mockapi.io/api/tccFake/jooj?search=${studentStr}`) // nome=${studentStr}
  ...
  const onCurrentTimeChange = (e: any) => {
    ...
    result.forEach(frame => {
      console.log(e.playedSeconds.toFixed(0), "=", parseFloat(frame.dt_cria).toFixed(0))
  ...
  return (
    <>
      <Input className="input" size="large" onChange={handleChange} onKeyPress={onKeyPress} value={nomeAluno} placeholder="Procurar Aluno..." prefix={<UserOutlined />} />
      <Container>
        <Layout>
          <Content>
            <Jooj className="jooj">
              ...
              <Descriptions title="Informações do Aluno">
                <Descriptions.Item label="Nome">{result[0].name}</Descriptions.Item>
                <Descriptions.Item label="RA">{result[0].ra}</Descriptions.Item>
                <Descriptions.Item label="Cidade">São Bernardo do Campo, São Paulo</Descriptions.Item>
              ...
              <h4 className="infoSlider" >
                Arraste para o lado
                <ArrowRightOutlined style={{ fontSize: '16px', color: 'black' }} />
              </h4>
              ...
              <Slider {...settings} className="container">
                ...
                {result.map((frame, i) => {
                  ...
                  if(index === 0)
                  console.log(result[i * intervalo])

                  if (result[i * intervalo] !== undefined) {
                    return (
                      <>
                        <img src={result[i * intervalo].url_frame} alt="emocao do aluno" className='imagem' />
                        <h2>{result[i * intervalo].emocao} - {result[i * intervalo].dt_cria}</h2>
                      </>)
                  }
                  return null
                })}
